import type { MapPerformance, MapReport } from '@fraglens/core';
...
export function renderMapReport(report: MapReport, theme: Theme, format: FormatOptions): string {
  return [
    ...
    ...section('MAPAS', mapTableLines(report.maps, report.minMapSample, theme), theme),
    rule(theme),
...
/** Tabela de desempenho por mapa com as notas de rodapé. */
export function mapTableLines(
  ...
  if (maps.some((map) => map.lowSample)) {
    notes.push(
      theme.colors.dim(
        `* Menos de ${minMapSample} partidas no mapa: amostra pequena, interprete com cuidado.`,
      ),
    );
  }
